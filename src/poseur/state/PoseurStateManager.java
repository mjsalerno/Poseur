...
public class PoseurStateManager {
    ...
    public void processMousePress(int x, int y) {
        ...
        if (state == PoseurState.CREATE_SHAPE_STATE) {
            ...
            //if the mouse was pressed in the bounds of a shape then 
            //mark that shape as selected and change the state to dragging
            this.selectedShape = pose.findShapeWithPoint(poseSpaceX, poseSpaceY);
            setState(PoseurState.DRAG_SHAPE_STATE);
        } else {
            //else make the selected shape null
            //and mark the state as selecting
            startShapeSelection();
